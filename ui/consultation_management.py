...
def _is_closing_stage(stage: str | None) -> bool:
    """새 상담의 종료와 과거 계약 완료 단계는 종료 입력 흐름으로 보인다."""
    return stage in {"종료", "계약 완료"}
...
def _split_standard_value(value: str | None, options: list[str]) -> tuple[list[str], str]:
    """과거 자유입력은 그대로 두고, 새 선택값과 기타 입력만 분리한다."""
    pieces = [item.strip() for item in (value or "").split("|") if item.strip()]
    selected = [item for item in pieces if item in options]
    other = " | ".join(item.removeprefix("기타:").strip() for item in pieces if item not in options)
    if other and "기타" not in selected:
        selected.append("기타")
    return selected, other


def _standard_value(selected: list[str], other: str) -> str | None:
    values = [item for item in selected if item != "기타"]
    if "기타" in selected and other.strip():
        values.append(f"기타: {other.strip()}")
    return " | ".join(values) or None


def _render_desired_condition_inputs(key_prefix: str, *, desired_area: str | None = None, desired_room_types: str | None = None) -> tuple[str | None, str | None]:
    saved_areas, other_area = _split_standard_value(desired_area, DESIRED_AREA_OPTIONS)
    saved_rooms, other_room = _split_standard_value(desired_room_types, DESIRED_ROOM_TYPE_OPTIONS)
    area_column, room_column = st.columns(2)
    with area_column:
        areas = st.multiselect("희망 지역", DESIRED_AREA_OPTIONS, default=saved_areas, key=f"{key_prefix}_areas")
        area_other = st.text_input("기타 희망 지역", value=other_area, disabled="기타" not in areas, key=f"{key_prefix}_area_other")
    with room_column:
        rooms = st.multiselect("희망 방 유형", DESIRED_ROOM_TYPE_OPTIONS, default=saved_rooms, key=f"{key_prefix}_rooms")
        room_other = st.text_input("기타 희망 방 유형", value=other_room, disabled="기타" not in rooms, key=f"{key_prefix}_room_other")
    return _standard_value(areas, area_other), _standard_value(rooms, room_other)
...
def _focus_badge(item: dict, today: date) -> str:
    if item["consultation_status"] == "종료" or item.get("progress_stage") == "계약 완료" or item.get("has_active_linked_contract"):
        return "-"
    if item.get("next_contact_date"):
        if item["next_contact_date"] < today.isoformat():
            return "다음 연락 지연"
        if item["next_contact_date"] == today.isoformat():
            return "다음 연락 오늘"
    try:
        days = (today - date.fromisoformat(item["consulted_date"])).days
    except (TypeError, ValueError):
        return "-"
    if 0 <= days <= 1:
        return "24시간 이내 1차 추천 확인"
    if 2 <= days <= 3:
        return "72시간 이내 추가 추천 확인"
    return "-"
...
def _rows(items: list[dict]) -> list[dict]:
    ...
    return [{
        "상담번호": consultation_number(item["consultation_id"]), "연결 매물번호": listing_number(item["listing_id"]), "고객 연락처": item["customer_phone"] or "-", "상담 구분": "일반 상담" if item["listing_id"] is None else "매물 상담", "건물명": item["building_name"] or "-", "지번주소": item["lot_address"] or "-", "호실": item["unit_number"] or "-",
        "매물 접수일": item["received_date"], "상담일": item["consulted_date"], "유입 경로": item["consultation_source"] or "-",
...
def _render_registration() -> None:
    ...
    registration_mode = st.radio("등록 방식", ["일반 상담", "매물 연결 상담"], horizontal=True, key="consultation_registration_mode")
    if registration_mode == "일반 상담":
        ...
        with st.form("general_consultation_create"):
            ...
            desired_area, desired_room_types = _render_desired_condition_inputs("general_create")
            desired_right, desired_last, desired_date_column = st.columns(3)
        ...
        if submitted:
            ...
            consultation, errors = validate_consultation({
                ...
                "consulted_date": consulted_date, "consultation_source": consultation_source, "consultation_note": note,
                ...
                "progress_stage": progress_stage, "closed_reason": None if closed_reason == "선택 안 함" else closed_reason, "desired_room_types": desired_room_types, "required_features_note": required_features_note,
            })
            if errors:
    ...
    with st.form(f"consultation_create_{selected['listing_id']}"):
        ...
        with right:
            ...
            consultation_source = st.selectbox("유입 경로", CONSULTATION_SOURCES)
        st.markdown("##### 희망 조건 (선택)")
        desired_area, desired_room_types = _render_desired_condition_inputs("listing_create")
        desired_right, desired_last, desired_date_column = st.columns(3)
        with desired_right: desired_deposit = st.number_input("희망 보증금 (만원)", min_value=0, step=100, value=None)
        with desired_last: desired_monthly_rent = st.number_input("희망 월세 (만원)", min_value=0, step=5, value=None)
        with desired_date_column: desired_available_from_date = st.date_input("희망 입주 가능일", value=None)
    ...
    if submitted:
        customer_phone = format_phone_number(customer_phone)
        consultation, errors = validate_consultation({
            "customer_phone": customer_phone, "consulted_date": consulted_date,
...
def _render_lookup() -> None:
    ...
    focus_only = st.checkbox("집중 대응 상담만 보기", key="consultation_focus_only")
    if focus_only:
        items = [item for item in items if _focus_badge(item, date.today()) != "-"]
    ...
    with st.form(f"consultation_edit_{detail['consultation_id']}"):
        ...
        with right:
            st.caption(f"상담 구분: {'일반 상담' if detail['listing_id'] is None else '매물 상담'}\n\n상담일: {detail['consulted_date']}")
        ...
        st.markdown("##### 희망 조건 (선택)")
        desired_area, desired_room_types = _render_desired_condition_inputs(f"consultation_edit_{detail['consultation_id']}", desired_area=detail["desired_area"], desired_room_types=detail["desired_room_types"])
        desired_right, desired_last, desired_date_column = st.columns(3)
    ...
    if submitted:
        try:
            ...
            save_consultation_changes(detail["consultation_id"], {
                "customer_name": detail["customer_name"], "customer_phone": customer_phone, "consultation_source": consultation_source, "consultation_note": note,
                "desired_area": desired_area,
                "desired_deposit_manwon": desired_deposit, "desired_monthly_rent_manwon": desired_monthly_rent,
                "desired_available_from_date": desired_available_from_date,
                "next_contact_date": None if clear_next_contact else next_contact, "consultation_status": status, "desired_room_types": desired_room_types,
            })
        except ValueError as error:
            st.error(str(error))
        else:
    ...
    if detail.get("linked_contract_exists"):
        ...
        if activities:
            ...
            with st.expander("후속 상담 이력 수정·삭제", expanded=False):
                ...
                if edit_activity_id in activity_options:
                    ...
                    edit_stage = st.selectbox("이번 상담 결과", edit_stage_options, index=edit_stage_options.index(selected_activity["stage_after_activity"]), format_func=_stage_label, key=f"activity_stage_{edit_activity_id}")
                    with st.form(f"consultation_activity_edit_{edit_activity_id}"):
                        ...
                        with edit_left:
                            ...
                            activity_options = [selected_activity["activity_type"], *ACTIVITY_TYPES] if selected_activity["activity_type"] not in ACTIVITY_TYPES else ACTIVITY_TYPES
                            edit_activity_type = st.selectbox("상담 방식", activity_options, index=activity_options.index(selected_activity["activity_type"]), key=f"activity_type_{edit_activity_id}")
                        with edit_middle:
                        ...
                        with edit_right:
                            if _is_closing_stage(edit_stage):
                                ...
                                edit_closed_reason = st.selectbox("종료 사유 *", ["선택 안 함", *CLOSED_REASONS], index=closed_index, key=f"activity_closed_reason_{edit_activity_id}")
                                edit_next_contact = None
                                st.caption("저장하면 상담 상태가 종료되고 다음 연락일은 자동으로 비워집니다.")
                            else:
                                edit_closed_reason = None
                                edit_next_contact = st.date_input("다음 연락일", value=date.fromisoformat(selected_activity["next_contact_date"]) if selected_activity["next_contact_date"] else None, key=f"activity_next_contact_{edit_activity_id}")
        ...
        stage_after_activity = st.selectbox("이번 상담 결과", stage_options, index=stage_options.index(detail["progress_stage"]), format_func=_stage_label, key=f"consultation_activity_stage_{detail['consultation_id']}")
        with st.form(f"consultation_activity_{detail['consultation_id']}"):
            ...
            with activity_left:
                ...
                activity_type = st.selectbox("상담 방식", ACTIVITY_TYPES)
            with activity_middle:
                visit_result = st.selectbox("방문 결과", ["미입력", *VISIT_RESULTS])
                st.caption("방문 상담이 아니면 `미입력`으로 두세요.")
            with activity_right:
                if _is_closing_stage(stage_after_activity):
                    closed_reason = st.selectbox("종료 사유 *", ["선택 안 함", *CLOSED_REASONS])
                    next_activity_contact = None
                    st.caption("저장하면 상담 상태가 종료되고 다음 연락일은 자동으로 비워집니다.")
                else:
                    closed_reason = None
                    next_activity_contact = st.date_input("다음 연락일", value=None)
        ...
        with st.expander("이 기존 상담 종료 처리"):
            st.caption("기존 상담 내용과 상담일은 바꾸지 않습니다. 종료 사유는 필요할 때만 남기고 다음 연락일만 해제합니다.")
            legacy_closed_reason = st.selectbox("종료 사유 (선택)", ["선택 안 함", *CLOSED_REASONS], key=f"legacy_close_reason_{detail['consultation_id']}")
            ...
            if st.button("기존 상담 종료 처리", type="secondary", disabled=not legacy_close_confirmed, key=f"legacy_close_{detail['consultation_id']}"):
                try:
                    close_legacy_consultation(detail["consultation_id"], None if legacy_closed_reason == "선택 안 함" else legacy_closed_reason)
                except ValueError as error:
                    st.error(str(error))
                else:
