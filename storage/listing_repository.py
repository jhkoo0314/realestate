...
def update_listing_quick_fields(listing_id: int, listing_status: str, has_listing_photos: str, next_check_date: str | None, cleaning_status: str | None, wallpaper_status: str | None, repair_status: str | None, path: Path = DATABASE_PATH) -> None:
    ...
    try:
        with connection:
            ...
            connection.execute("UPDATE listings SET listing_status=?, has_listing_photos=?, next_check_date=?, cleaning_status=?, wallpaper_status=?, repair_status=? WHERE id=?",(listing_status,has_listing_photos,next_check_date,cleaning_status,wallpaper_status,repair_status,listing_id))
    finally: connection.close()
