...
def get_contracts(*, query: str = "", statuses: list[str] | None = None, end_start: str | None = None, end_end: str | None = None, expiring_within_days: int | None = None, unit_id: int | None = None, path: Path = DATABASE_PATH) -> list[dict[str, Any]]:
    ...
    try:
        rows = connection.execute(f"""
            SELECT c.id AS contract_id, c.listing_id, c.contract_type, c.contract_progress_date, c.formal_contract_date, c.contract_start_date, c.contract_end_date, c.term_months, c.contract_status, c.contract_note, c.contractor_contact, c.contract_deposit_manwon, c.provisional_deposit_manwon, c.remaining_deposit_due_date, c.balance_manwon, c.created_at, c.updated_at, b.building_name, b.lot_address, u.unit_number, l.received_date, l.listing_status
...
def create_contract(listing_id: int, contract: dict[str, Any], path: Path = DATABASE_PATH) -> int:
    ...
    try:
        with connection:
            ...
            cursor = connection.execute("""INSERT INTO contracts (listing_id, contract_type, contract_progress_date, formal_contract_date, contract_start_date, contract_end_date, term_months, contract_status, contract_note, contractor_contact, contract_deposit_manwon, provisional_deposit_manwon, remaining_deposit_due_date, balance_manwon) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (listing_id, contract["contract_type"], contract.get("contract_progress_date"), contract.get("formal_contract_date"), contract.get("contract_start_date"), contract.get("contract_end_date"), contract.get("term_months"), contract["contract_status"], contract.get("contract_note"), contract.get("contractor_contact"), contract.get("contract_deposit_manwon"), contract.get("provisional_deposit_manwon"), contract.get("remaining_deposit_due_date"), contract.get("balance_manwon")))
