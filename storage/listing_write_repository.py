...
def update_current_listing(listing_id: int, listing: dict[str, Any], path: Path = DATABASE_PATH) -> None:
    ...
    try:
        with connection:
            ...
            connection.execute("""UPDATE listings SET received_date=?,listing_status=?,closed_date=NULL,close_reason=NULL,deposit_manwon=?,monthly_rent_manwon=?,management_fee_manwon=?,availability_type=?,move_out_due_date=?,listing_holder=?,listing_note=?,next_check_date=?,landlord_contact=?,tenant_contact=? WHERE id=?""",(listing.get("received_date", date.today().isoformat()),listing["listing_status"],listing["deposit_manwon"],listing["monthly_rent_manwon"],listing.get("management_fee_manwon"),listing["availability_type"],listing.get("move_out_due_date"),listing.get("listing_holder"),listing.get("listing_note"),listing.get("next_check_date"),listing.get("landlord_contact"),listing.get("tenant_contact"),listing_id))
