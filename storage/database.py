...
def _ensure_compatibility_columns(connection: sqlite3.Connection) -> None:
    ...
    connection.executescript(SCHEMA)
    existing_listings = {row[1] for row in connection.execute("PRAGMA table_info(listings)")}
    if "advertised_platforms" not in existing_listings:
        connection.execute("ALTER TABLE listings ADD COLUMN advertised_platforms TEXT")
    return
