...
현재 **13단계: 데이터 저장 코드 기능별 분리**까지 코드 구현과 기본 자동 확인을 마쳤습니다. 10단계의 전체 백업·복구·사무실 운영 검증은 사용자 요청으로 보류 중입니다. 각 단계의 실제 브라우저 업무 확인은 아직 남아 있습니다.
...
- 매물 회차별 일반·단기 계약의 계약 진행일·정식 계약일·임대차 기간·상태 누적 관리와 계약 만료 30일 전 조회
- 계약금 전체·가계약금 수령액·추가 수령 예정일 및 미수령액 표시
...
- 매물·계약·상담을 저장·수정·삭제하면 무결성 검사를 통과한 날짜별 SQLite 사본을 OneDrive `Documents\매물관리백업\daily`에 자동 저장하며, 최근 10일만 유지합니다.
- 매물 현황 리스트는 조회·필터·내보내기용이며, 새 등록·최신 정보 수정·종료는 `매물 등록·수정` 메뉴에서 처리합니다.
